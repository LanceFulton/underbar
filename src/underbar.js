(function() {
  ...
  _.reduce = function(collection, iterator, accumulator) {
    if (typeof accumulator !== 'undefined'){
      for (var i=0 ; i<collection.length ; i++) {
        var tempResult = iterator(accumulator, collection[i]);
  ...
  _.every = function(collection, iterator) {
    ...
    if (truths.length === collection.length) {
      return true;
    } else {
      return false;
    }
  ...
  _.defaults = function(obj) {
    for (var i=1 ; i<arguments.length ; i++){   /*cycles through arguments*/
      for (var key in arguments[i]){            /*cycles through keys in argument*/
  ...
  _.delay = function(func, wait) {
    var trueArgs = [];
    for (var i=0 ; i<arguments.length ; i++){
      trueArgs.push(arguments[i]);
    }
    trueArgs.splice(0,2);
    return setTimeout(function(){
      return func.apply(this, trueArgs);
    }, wait);
  };
